...
v0.7.0 新增独立管理员后台与用户运营基础。后台仅供数据库中已授权的管理员账号使用，普通用户看不到入口，也不能通过手动访问后台 URL 进入。
...
- 独立管理员后台：用户概览、VIP 测试权限管理、反馈处理和操作记录
- `/health` 健康检查接口
...
scripts/set_admin.py      管理员授权/撤销脚本
...
生产部署前请先备份 SQLite 数据库。可以使用已有备份脚本，也可以按部署环境的存储策略复制数据库文件：
...
v0.7.0 会为旧数据库安全补充以下字段：

- `users.is_admin`：管理员身份，旧用户默认 `0`
- `users.vip_status`：VIP 测试权限状态，默认 `free`
- `users.vip_expires_at`：VIP 测试权限到期时间，可为空
- `users.last_login_at`：最后成功登录时间，可为空
- `users.is_active`：账号状态，默认 `1`
- `feedback.admin_note`：管理员内部备注，默认空字符串
...
后台仅供已授权管理员使用。未登录用户访问后台会跳转登录页；已登录但未授权的普通用户会得到 403。

管理员授权只通过命令行脚本完成，不提供网页端“成为管理员”入口，不创建默认管理员账号，也不写死密码。
...
```bash
python scripts/set_admin.py <username> --remove
```

部署后建议执行一次数据库初始化，再授权管理员：
...
当前 VIP 功能是“VIP 测试权限”，用于小范围运营测试，不代表正式付费会员服务，也没有接入支付或订单系统。
